...
public class KategoriPengeluaranFrame extends JPanel {
  ...
  private void delete(){int r=selected(); if(r>=0 && JOptionPane.showConfirmDialog(this,"Hapus kategori pengeluaran?","Konfirmasi",JOptionPane.YES_NO_OPTION)==JOptionPane.YES_OPTION) try{controller.deleteKategori((int)model.getValueAt(r,0)); loadData();}catch(Exception e){error(e);}}
  private void dialog(int id,String value){JDialog d=new JDialog(SwingUtilities.getWindowAncestor(this),id==0?"Tambah Kategori Pengeluaran":"Edit Kategori Pengeluaran",Dialog.ModalityType.APPLICATION_MODAL); JPanel p=UIUtils.card(); p.setLayout(new GridBagLayout()); GridBagConstraints g=new GridBagConstraints(); g.insets=new Insets(8,8,8,8); g.fill=GridBagConstraints.HORIZONTAL; JTextField nama=UIUtils.textField(24); nama.setText(value); g.gridx=0;g.gridy=0;p.add(UIUtils.formLabel("Nama Kategori"),g);g.gridx=1;p.add(nama,g); JButton save=UIUtils.button("Simpan",UIUtils.GREEN); g.gridx=0;g.gridy=1;g.gridwidth=2;p.add(save,g); save.addActionListener(e->{try{ if(id==0) controller.insertKategori(nama.getText().trim()); else controller.updateKategori(id,nama.getText().trim()); d.dispose(); loadData(); }catch(Exception ex){error(ex);}}); d.add(p); d.pack(); d.setLocationRelativeTo(this); d.setVisible(true);}
